...
void findPathS(char letterMap[][COL], int xStart, int yStart, int xGoal, int yGoal, double we, int mNum, int inNum, double we2)
{
	Fringe * allFringe[4];
	for (int i = 0; i < 4; i++) {
	...
	goalNode->setGoal(letterMap[xGoal][yGoal], xGoal, yGoal, 0.25);
	startNode->setStart(letterMap[xStart][yStart], xStart, yStart, goalNode, 0.25, 1);
	startNode->gVal = 0;
	startNode->calcH(goalNode, 0.25, 1);
	startNode->prev = startNode;
	node * rtI = new node;
	rtI->setI(startNode->xCord, startNode->yCord, startNode);
	cl0->insertI(rtI);
	fringe0->insert(startNode, startNode->getG() + startNode->getH());
	for (int i = 0; i < 4; i++) {
		allFringe[i]->insert(startNode, startNode->getG() + startNode->getH());
	}
	...
	while (fringe0->peek() < 100000) {
		...
		for (int i = 0; i < 4; i++) {
			if (allFringe[i]->peek() <= fringe0->peek()*we2) {
				...
				if (curSi->xCord == goalNode->xCord && curSi->yCord == goalNode->yCord) {
					...
					node * ptr2 = cl[i]->headI;
					ptr2->calcH(goalNode, we, i + 2);
					ofstream cliFile;
					...
					if (letterMap[curSi->xCord + 1][curSi->yCord] != '0' &&curSi->xCord + 1 <= ROW - 1) {                                        // |--
						...
						if (!cl[i]->isOn(rtI)) {
							...
							rtNode->prev = curSi;
							rtI->prev = curSi;
							rtNode->setNode(letterMap[curSi->xCord + 1][curSi->yCord], curSi->xCord + 1, curSi->yCord, goalNode, we, i + 2);
							cl[i]->insertI(rtI);
					...
					if (letterMap[curSi->xCord + 1][curSi->yCord - 1] != '0'&&curSi->xCord + 1 <= ROW - 1 && curSi->yCord - 1 >= 0) {                                     //  "\_"
						...
						if (!cl[i]->isOn(rtdwI)) {
							...
							rtdwNode->prev = curSi;
							rtdwI->prev = curSi;
							rtdwNode->setNode(letterMap[curSi->xCord + 1][curSi->yCord - 1], curSi->xCord + 1, curSi->yCord - 1, goalNode, we, i + 2);
							cl[i]->insertI(rtdwI);
					...
					if (letterMap[curSi->xCord][curSi->yCord - 1] != '0' && curSi->yCord - 1 >= 0) {                                       // |_
						...
						if (!cl[i]->isOn(dwI)) {
							...
							dwNode->prev = curSi;
							dwI->prev = curSi;
							dwNode->setNode(letterMap[curSi->xCord][curSi->yCord - 1], curSi->xCord, curSi->yCord - 1, goalNode, we, i + 2);
							cl[i]->insertI(dwI);
					...
					if (letterMap[curSi->xCord - 1][curSi->yCord - 1] != '0'&&curSi->xCord - 1 >= 0 && curSi->yCord - 1 >= 0) {                                   // _/
						...
						if (!cl[i]->isOn(lfdwI)) {
							...
							lfdwNode->prev = curSi;
							lfdwI->prev = curSi;
							lfdwNode->setNode(letterMap[curSi->xCord - 1][curSi->yCord - 1], curSi->xCord - 1, curSi->yCord - 1, goalNode, we, i + 2);
							cl[i]->insertI(lfdwI);
					...
					if (letterMap[curSi->xCord - 1][curSi->yCord] != '0'&&curSi->xCord - 1 >= 0) {                                      // --|.
						...
						if (!cl[i]->isOn(lfI)) {
							...
							lfNode->prev = curSi;
							lfI->prev = curSi;
							lfNode->setNode(letterMap[curSi->xCord - 1][curSi->yCord], curSi->xCord - 1, curSi->yCord, goalNode, we, i + 2);
							cl[i]->insertI(lfI);
					...
					if (letterMap[curSi->xCord - 1][curSi->yCord + 1] != '0'&& curSi->xCord - 1 >= 0 && curSi->yCord + 1 <= COL - 1) {                             // --"\"
						...
						if (!cl[i]->isOn(lftpI)) {
							...
							lftpNode->prev = curSi;
							lftpI->prev = curSi;
							lftpNode->setNode(letterMap[curSi->xCord - 1][curSi->yCord + 1], curSi->xCord - 1, curSi->yCord + 1, goalNode, we, i + 2);
							cl[i]->insertI(lftpI);
					...
					if (letterMap[curSi->xCord][curSi->yCord + 1] != '0'&& curSi->yCord + 1 <= COL - 1) {                            //  ^|^
						...
						if (!cl[i]->isOn(tpI)) {
							...
							tpNode->prev = curSi;
							tpI->prev = curSi;
							tpNode->setNode(letterMap[curSi->xCord][curSi->yCord + 1], curSi->xCord, curSi->yCord + 1, goalNode, we, i + 2);
							cl[i]->insertI(tpI);
					...
					if (letterMap[curSi->xCord + 1][curSi->yCord + 1] != '0'&&curSi->xCord + 1 <= ROW - 1 && curSi->yCord + 1 <= COL - 1) {                                  // /--
						...
						if (!cl[i]->isOn(rttpI)) {
							...
							rttpNode->prev = curSi;
							rttpI->prev = curSi;
							rttpNode->setNode(letterMap[curSi->xCord + 1][curSi->yCord + 1], curSi->xCord + 1, curSi->yCord + 1, goalNode, we, i + 2);
							cl[i]->insertI(rttpI);
				...
				curS0 = fringe0->pop();
				if (curS0->xCord == goalNode->xCord && curS0->yCord == goalNode->yCord) {
					...
					if (letterMap[curS0->xCord + 1][curS0->yCord] != '0' &&curS0->xCord + 1 <= ROW - 1) {                                        // |--
						...
						if (!cl0->isOn(rtI)) {
							...
							rtNode->prev = curS0;
							rtI->prev = curS0;
							rtNode->setNode(letterMap[curS0->xCord + 1][curS0->yCord], curS0->xCord + 1, curS0->yCord, goalNode, we, 1);
							cl0->insertI(rtI);
					...
					if (letterMap[curS0->xCord + 1][curS0->yCord - 1] != '0'&&curS0->xCord + 1 <= ROW - 1 && curS0->yCord - 1 >= 0) {                                     //  "\_"
						...
						if (!cl0->isOn(rtdwI)) {
							...
							rtdwNode->prev = curS0;
							rtdwI->prev = curS0;
							rtdwNode->setNode(letterMap[curS0->xCord + 1][curS0->yCord - 1], curS0->xCord + 1, curS0->yCord - 1, goalNode, we, 1);
							cl0->insertI(rtdwI);
					...
					if (letterMap[curS0->xCord][curS0->yCord - 1] != '0' && curS0->yCord - 1 >= 0) {                                       // |_
						...
						if (!cl0->isOn(dwI)) {
							...
							dwNode->prev = curS0;
							dwI->prev = curS0;
							dwNode->setNode(letterMap[curS0->xCord][curS0->yCord - 1], curS0->xCord, curS0->yCord - 1, goalNode, we, 1);
							cl0->insertI(dwI);
					...
					if (letterMap[curS0->xCord - 1][curS0->yCord - 1] != '0'&&curS0->xCord - 1 >= 0 && curS0->yCord - 1 >= 0) {                                   // _/
						...
						if (!cl0->isOn(lfdwI)) {
							...
							lfdwNode->prev = curS0;
							lfdwI->prev = curS0;
							lfdwNode->setNode(letterMap[curS0->xCord - 1][curS0->yCord - 1], curS0->xCord - 1, curS0->yCord - 1, goalNode, we, 1);
							cl0->insertI(lfdwI);
					...
					if (letterMap[curS0->xCord - 1][curS0->yCord] != '0'&&curS0->xCord - 1 >= 0) {                                      // --|.
						...
						if (!cl0->isOn(lfI)) {
							...
							lfNode->prev = curS0;
							lfI->prev = curS0;
							lfNode->setNode(letterMap[curS0->xCord - 1][curS0->yCord], curS0->xCord - 1, curS0->yCord, goalNode, we, 1);
							cl0->insertI(lfI);
					...
					if (letterMap[curS0->xCord - 1][curS0->yCord + 1] != '0'&& curS0->xCord - 1 >= 0 && curS0->yCord + 1 <= COL - 1) {                             // --"\"
						...
						if (!cl0->isOn(lftpI)) {
							...
							lftpNode->prev = curS0;
							lftpI->prev = curS0;
							lftpNode->setNode(letterMap[curS0->xCord - 1][curS0->yCord + 1], curS0->xCord - 1, curS0->yCord + 1, goalNode, we, 1);
							cl0->insertI(lftpI);
					...
					if (letterMap[curS0->xCord][curS0->yCord + 1] != '0'&& curS0->yCord + 1 <= COL - 1) {                            //  ^|^
						...
						if (!cl0->isOn(tpI)) {
							...
							tpNode->prev = curS0;
							tpI->prev = curS0;
							tpNode->setNode(letterMap[curS0->xCord][curS0->yCord + 1], curS0->xCord, curS0->yCord + 1, goalNode, we, 1);
							cl0->insertI(tpI);
					...
					if (letterMap[curS0->xCord + 1][curS0->yCord + 1] != '0'&&curS0->xCord + 1 <= ROW - 1 && curS0->yCord + 1 <= COL - 1) {                                  // /--
						...
						if (!cl0->isOn(rttpI)) {
							...
							rttpNode->prev = curS0;
							rttpI->prev = curS0;
							rttpNode->setNode(letterMap[curS0->xCord + 1][curS0->yCord + 1], curS0->xCord + 1, curS0->yCord + 1, goalNode, we, 1);
							cl0->insertI(rttpI);
